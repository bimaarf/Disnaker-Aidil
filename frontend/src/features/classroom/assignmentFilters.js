...
// Main AssignmentFilters component
const AssignmentFilters = ({
  ...
  const handleSearchChange = useCallback(
    (value) => {
      ...
      searchTimeoutRef.current = setTimeout(() => {
        fetchAssignmentsData({ search: value });
      }, 500);
    },
    [fetchAssignmentsData]
  );
  ...
  const handleSearchClear = useCallback(() => {
    ...
    fetchAssignmentsData({ search: "" });
  }, [fetchAssignmentsData]);

  // Filter handlers
  const handleTypeChange = useCallback(
    (type) => {
      fetchAssignmentsData({ type });
  ...
  const handleSortChange = useCallback(
    (sortBy, sortOrder) => {
      fetchAssignmentsData({ sort_by: sortBy, sort_order: sortOrder });
      setIsSortOpen(false);
    },
    [fetchAssignmentsData]
  );
  ...
  const handlePerPageChange = useCallback(
    (perPage) => {
      fetchAssignmentsData({ per_page: perPage });
  ...
  const handleRefresh = useCallback(() => {
    fetchAssignmentsData(filters, { forceRefresh: true });
  }, [fetchAssignmentsData, filters]);
