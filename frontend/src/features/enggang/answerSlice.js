...
const getStatusFromResult = (status, is_approve, selection_type) => {
  console.log(
    `[getStatusFromResult] status=${status}, is_approve=${is_approve}, selection_type=${selection_type}`
  );
  if (status === true) return "Lulus";
  if (status === false && selection_type !== null) return "Tidak_Lulus";
  if (is_approve === true) return "Berkas_Diterima";
  if (is_approve === false) return "Berkas_Dikembalikan";
...
export const fetchRespondentByKey = createAsyncThunk(
  ...
  async (key, { rejectWithValue, getState }) => {
    try {
      ...
      const result = {
        data: {
          ...toPlainObject(response.data.data), // ✅ SANITIZED OBJECT
...
export const submitAnswers = createAsyncThunk(
  ...
  async (formData, { rejectWithValue, dispatch, getState }) => {
    try {
      ...
      // Enhanced dispatch to ensure proper cache updates
      dispatch({
      ...
      // Get current state and filters
      const state = getState();
      const filters = state.answers.filters;
      const periodId = newSubmission.answers?.[0]?.period_id;

      // Update all relevant caches immediately
      const plainSubmission = toPlainObject(newSubmission);
      ...
      await dispatch(
        fetchAnswerGroup({
          ...
          fromDate: filters.fromDate,
          toDate: filters.toDate,
          fromCache: false, // Force fresh fetch
        })
      );
  ...
  reducers: {
      ...
      state.respondentsByPeriod[periodId] = {
        ...
        data:
          current_page === 1
            ? data
            : [
                ...(current(state.respondentsByPeriod[periodId]?.data) || []),
                ...data,
              ],
        current_page,
        last_page,
    ...
    addNewSubmission: (state, action) => {
      ...
      if (state.respondents.some((r) => r.submission_id === submissionId)) {
        console.debug(
          `[answerSlice] Submission ${submissionId} already exists, skipping`
        );
        return;
      }
      ...
      // Tambahkan ke respondentsByPeriod
      const periodId = newSubmission.answers?.[0]?.period_id;
      if (periodId) {
