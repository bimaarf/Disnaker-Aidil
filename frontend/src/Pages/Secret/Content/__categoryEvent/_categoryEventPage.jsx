...
import {
  ...
  handleDeleteData,
  handleEditData,
} from "./__actions/__categoryEventAction"; // Import the actions
import { BadgeCategoryEvents } from "./__badgeCategoryEvent";
import CategoryEventList from "./__components/_categoryEventList";
import CategoryEventTable from "./__components/_categoryEventTable";
import { fetchCategoryEvents } from "../../../../features/event/categoryEventSlice";

export const CategoryEventPage = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const datas = useSelector((state) => state.categoryEvents.categoryEvents);
  const total = useSelector((state) => state.categoryEvents.total);
  const status = useSelector((state) => state.categoryEvents.status);
  const page = useSelector((state) => state.categoryEvents.page);
  const totalPages = useSelector((state) => state.categoryEvents.totalPages);
  const totalVisible = useSelector((state) => state.categoryEvents.totalVisible);
  const totalHidden = useSelector((state) => state.categoryEvents.totalHidden);
  ...
  useEffect(() => {
    dispatch(fetchCategoryEvents({ page: currentPage, perPage: 10 }))
      .unwrap()
  ...
  const handlePreviewData = (data) => {
    ...
    if (!isMobile) {
      ...
      navigate(`/category/event/preview/${data.key}`);
    }
  };
  ...
  return (
    <div className="min-h-[90vh]">
      <div
        ...
        onClick={() => navigate("/category/event/create")}>
