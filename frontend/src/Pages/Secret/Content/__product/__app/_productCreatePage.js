...
import { formatRupiah, parseRupiahToInt } from "../../../../../utils/rupiahInput";
...
const ProductCreatePage = () => {
  ...
  // Memoize unique categories and promotions
  ...
  const uniquePromotions = useMemo(
    () =>
      Array.from(
        new Map(promotions.map((promotion) => [promotion.id, promotion])).values()
      ),
    [promotions]
  );

  // Fetch categories and promotions only once on mount
  useEffect(() => {
    let mounted = true;
    ...
    const fetchData = async () => {
      try {
        if (categories.length === 0) {
          await dispatch(
            fetchCategoryProducts({ pageFilter: 1, perPageFilter: 100 })
          ).unwrap();
        }
        if (promotions.length === 0) {
          await dispatch(
            fetchPromotionProducts({ pageFilter: 1, perPageFilter: 100 })
          ).unwrap();
        }
      } catch (err) {
        if (mounted) {
  ...
  const handlePromotionChange = useCallback((promotionId) => {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      if (name.length > 255) {
        toast.error("Product name must not exceed 255 characters");
        return;
      }
      if (!description.trim()) {
      ...
      if (images.length > 0 && !images.some((img) => img.is_primary)) {
        toast.error("Please set one image as primary");
        return;
      }
      ...
      try {
        ...
        selectedPromotions.forEach((promotionId, index) => {
          formData.append(`promotion_ids[${index}]`, promotionId);
        });
        ...
        let errorMessage = "Unknown error occurred while creating product";
                        ...
                        {uniquePromotions.map((promotion) => (
                          ...
                          >
                            <input
                              ...
                              onChange={() => handlePromotionChange(promotion.id)}
                              className="checkbox checkbox-primary"
                            />
                            <span className="text-sm font-medium text-base-content">
                              {promotion.title}
