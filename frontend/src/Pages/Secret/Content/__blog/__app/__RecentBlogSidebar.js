    ...
    <article
      ...
      <div className="relative flex-shrink-0 w-20 h-20 overflow-hidden rounded-lg">
        ...
        {(blog.views > 500 || blog.featured) && (
          <div className="absolute top-1 right-1 bg-gradient-to-r from-[#1F1F1F] to-[#1F1F1F] text-white px-1.5 py-0.5 rounded text-xs font-bold">
      ...
      <div className="flex-1 min-w-0">
        <h3 className="text-sm font-semibold text-[#1F1F1F] line-clamp-2 group-hover:text-[#1F1F1F] transition-colors leading-tight mb-1">
          {blog.name}
        </h3>
        <div
  ...
  return (
    <>
      <div className="my-6 print-hidden">
        <div className="flex items-center gap-2 text-white bg-gradient-to-r from-[#1F1F1F] via-[#1F1F1F] to-[#1F1F1F] backdrop-blur-sm px-4 py-3 rounded shadow-sm overflow-x-auto ">
          <span
      ...
      <div className="bg-white rounded shadow-sm backdrop-blur-sm overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-3 py-2 text-sm text-gray-700 bg-gray-50 rounded border border-gray-300 focus:border-[#1F1F1F] outline-none transition-all landing-input"
            />
          </div>
        </div>
          ...
          {status === "loading" && (
            <div className="flex justify-center items-center py-8">
              <div className="relative w-10 h-10">
                ...
                <div className="absolute inset-0 border-3 border-[#1F1F1F] border-t-transparent rounded-full animate-spin"></div>
              </div>
            </div>
          )}
        ...
        {recentBlogs.length > 0 && (
          <div className="p-4 bg-gray-50 border-t border-gray-200">
            <button
              onClick={() => navigate("/blogs")}
              className="w-full text-center text-sm font-semibold text-[#1F1F1F] hover:text-gray-700 transition-colors py-2">
              Lihat Semua Berita →
            </button>
          </div>
        )}
