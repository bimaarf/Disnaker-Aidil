...
const ASSIGNMENT_TYPES = [
  ...
  // },
  { value: "form", label: "Kuis", icon: Sword, color: "text-orange-600" },
];
...
// UI Components (reuse from AssignmentDetailPage where possible)
      ...
      <div className="p-8">
        <div className="text-center py-16">
          ...
          <p className="text-base-content/60">
            Sedang mengambil informasi tugas untuk diedit...
          </p>
        </div>
      </div>
    </div>
...
const AssignmentUpdatePage = () => {
  ...
  // Assignment detail hook (unchanged)
  const {
    assignment,
    isLoading,
    error,
    ...
    cachedData,
    fetchAttempts = 0,
  } = useAssignmentDetail(classroomCode, assignmentId, {
    autoLoad: true,
    enablePolling: false,
  ...
  const draftKey = `assignment_draft_${classroomCode}_${assignmentId}`;
  ...
  useEffect(() => {
    const dataSource = assignment || cachedData;
    if (
      dataSource &&
  ...
  }, [assignment, cachedData, draftKey]);
  ...
  const handleBack = useCallback(() => {
    ...
    navigate(`/classrooms/${classroomCode}/assignment/${assignmentId}/preview`);
  }, [navigate, classroomCode, assignmentId, isDirty]);
    ...
    async (event) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          initialDataRef.current = {
            ...formData,
            existingFiles: updatedAssignment.file_urls || existingFiles,
            links: allCurrentLinks.length > 0 ? allCurrentLinks : [""],
            updated_at: updatedAssignment.updated_at,
          };
          localStorage.removeItem(draftKey);
          // navigate(
          //   `/classrooms/${classroomCode}/assignment/${assignmentId}/preview`
          // );
        }
      } catch (err) {
  ...
  useEffect(() => {
    const dataSource = hasCachedData && cachedData ? cachedData : assignment;
  ...
  }, [assignment, hasCachedData, cachedData]);
  ...
  }, [classroomCode, assignmentId, navigate]);
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <nav className="text-sm breadcrumbs">
            <ul className="flex items-center gap-2 text-base-content/60">
              ...
              <li>
                <RouterLink
                  to={`/classrooms/${classroomCode}/assignment/${assignmentId}/preview`}
                  className="hover:text-primary transition-colors">
                  Detail Tugas
                </RouterLink>
              </li>
              <li className="text-base-content/40">/</li>
              <li className="text-base-content font-medium">Edit Tugas</li>
            </ul>
          </nav>
        </div>
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-base-content/60 mt-2">
                    Perbarui informasi tugas pembelajaran
                  </p>
                </div>
              </div>
          ...
          <div className="bg-base-100 dark:bg-base-200 backdrop-blur-sm rounded-2xl shadow-sm border border-base-200/50 p-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-[11px] text-base-content/60 mb-4">
                Pilih file untuk ditambahkan ke tugas (Maks. 40MB per file)
              </p>
            </div>
          </div>
          {/* Link */}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 p-4 bg-base-200 dark:bg-base-100 rounded-xl transition-all duration-300">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm font-bold text-base-content">
                    {formatDateDisplay(assignment?.created_at)}
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <ActionButton
                onClick={() => {
                  ...
                  saveAs(blob, `assignment-${assignmentId}-draft.xlsx`);
