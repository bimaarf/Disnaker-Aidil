...
import { useAssignmentDetail } from "../../../../../../features/classroom/assignmentHook";
...
import SubmissionAssignmentDetail from "./_submissionAssignmentDetail";
import AssignmentQuestionManager from "./components/_assignmentQuestion/_assignmentQuestionManager";
import AssignmentAnswerResponder from "./components/_assignmentQuestion/AssignmentAnswerResponder";
...
// PERBAIKAN: Fungsi helper untuk check apakah file bisa di-view
    ...
    return (
      <div style={style}>
        <div
          ...
          className={`group cursor-pointer rounded-lg transition-all duration-200 border mx-2 ${
            isActive
      ...
      <div className="w-full ">
        {videos.length === 0 ? (
          ...
          <FixedSizeList
            ...
            itemSize={80} // Approximate height of each video item
...
const EmptyState = memo(({ onBack, onRefresh }) => (
  <div className="w-full mx-auto min-h-screen px-1 md:px-0">
      ...
      <div className="p-4">
        <div className="text-center py-16">
          ...
          <p className="text-base-content/60  max-w-md mx-auto">
            Tugas yang Anda cari tidak dapat ditemukan. Mungkin telah dihapus
            atau tidak tersedia.
          </p>
      ...
      <div className="p-4">
        <div className="text-center py-16">
          ...
          <p className="text-base-content/60">
            Sedang mengambil informasi tugas pembelajaran...
          </p>
        </div>
      </div>
    </div>
...
// ==================== OPTIMIZED ASSIGNMENT CONTENT COMPONENT ====================
const AssignmentContent = memo(({ assignment, showMore, onToggleMore }) => {
  // Hook khusus untuk code blocks Quill saja
  const { elementRef } = useQuillCodeCopy([assignment?.description, showMore]);
    ...
    <div className="mb-6 relative">
      ...
      <div className="relative p-4 rounded-2xl bg-base-100 dark:bg-base-200 backdrop-blur-sm shadow-sm border border-base-300/20">
        ...
        <div className="text-sm text-base-content/80">
          ...
          {assignment.description && assignment.description.length > 300 && (
            <button
...
const VideoSection = memo(({ videoFiles, onViewUpdate }) => {
  ...
  return (
    <div className="">
      ...
      <h2 className="text-lg font-bold text-base-content flex items-center gap-2 mb-4">
        ...
        Video Pembelajaran ({videoFiles.length})
      </h2>
...
const AssignmentDetailPage = () => {
  const { code: classroomCode, assignmentId } = useParams();
  const navigate = useNavigate();
  const currentUser = useSelector(selectUser);
  const currentUserRole = useSelector(selectUserRoles);
  const [selectedVideo, setSelectedVideo] = useState(null);
  const isMobile = useIsMobile();
  // Move ALL useState hooks to the top, before any conditions
  const [viewModeContent, setViewModeContent] = useState(VIEW_MODES.ASSIGNMENT);
  ...
  const hasRequiredParams = classroomCode && assignmentId;
  // Optimized file filtering with memoization

  useEffect(() => {
    if (!hasRequiredParams) {
      console.error("[AssignmentDetailPage] Missing required params:", {
        classroomCode,
        assignmentId,
      });
      navigate("/classrooms", { replace: true });
    }
  }, [hasRequiredParams, classroomCode, assignmentId, navigate]);
  ...
  const {
    ...
    fetchAttempts = 0,
  } = useAssignmentDetail(classroomCode, assignmentId, hookOptions);

  const { videoFiles, nonVideoFiles } = useMemo(() => {
    const files = assignment?.file_urls || [];
    return {
  ...
  }, [assignment, isLoading, error, hasCachedData, fetchAttempts]);
  ...
  }, [navigate, classroomCode, assignmentId]);
  ...
  const handleDownloadClick = useCallback(
    ...
    [handleDownloadFile, assignment, downloadFileDirectly]
  );
  ...
  const handleViewClick = useCallback(
    ...
    [assignment, handleViewExternal, handleDownloadClick]
  );
  ...
  const handleViewExternalClick = useCallback(
    async (linkUrl) => {
      ...
      const linkObject = assignment.links?.find((link) => link.url === linkUrl);
  ...
  const handleVideoViewUpdate = useCallback(
    async (fileId) => {
      try {
        // Let the global tracker handle duplicate prevention
        const result = await handleViewExternal(fileId);

        if (result?.success) {
          console.log(
            "✅ Video view count updated via assignment handler for file:",
            fileId
          );
        }
        ...
        console.error(
          "❌ Failed to update video view count via assignment handler:",
          error
        );
        throw error;
      }
  ...
  if (!assignment && derivedState.isInitialized && !isLoading) {
          ...
          <div className="flex justify-between">
            <div className="flex items-start md:items-center gap-4">
              <div
                ...
                <TypeIcon
                  className={`w-8 h-8 ${getTypeColor(assignment.type)}`}
                />
              </div>
              <div>
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 text-xs text-base-content/70 mb-4">
              <div className="flex items-center gap-2 px-2 py-1 bg-base-100/50 rounded-full">
                ...
                <span>Dibuat {formatDateRelative(assignment.created_at)}</span>
              </div>
              {assignment.updated_at !== assignment.created_at && (
                <div className="flex items-center gap-2 px-2 py-1 bg-base-100/50 rounded-full">
                  <Clock className="w-3 h-3" />
                  <span>
                    Diperbarui {formatDateRelative(assignment.updated_at)}
                  </span>
                </div>
              )}
              {assignment.uploader && (
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {assignment.links?.length > 0 && (
                <ActionButton
                  onClick={() =>
                    handleViewExternalClick(assignment.links[0].url)
                  }
                  icon={ExternalLink}
                  label={`Buka Link ${currentUserRole}`}
                  color="info"
                />
              )}
            </div>
      ...
      <div className="grid gap-4 md:gap-4 grid-cols-1 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <StatCard
              ...
              label="Dilihat"
              value={
                Array.isArray(assignment?.file_urls)
                  ? assignment.file_urls
            ...
            <StatCard
              icon={Star}
              label="File Tugas"
              value={assignment.file_urls?.length || 0}
              color="yellow"
            />
            <StatCard
              icon={Video}
          ...
          {/* View Mode Toggle */}
          <div className="flex w-fit justify-end gap-2 items-center bg-base-100/80 backdrop-blur-sm shadow-sm rounded-2xl p-1.5 border border-base-200/50">
            <button
              onClick={() => setViewModeContent(VIEW_MODES.ASSIGNMENT)}
              className={`flex items-center gap-2 p-3 rounded-xl transition-all duration-200 transform whitespace-nowrap  hover:scale-[99%] active:scale-[98%] ${
                viewModeContent === VIEW_MODES.ASSIGNMENT
            ...
            <button
              onClick={() => setViewModeContent(VIEW_MODES.SUBMISSION)}
              className={`flex items-center gap-2 p-3 rounded-xl transition-all duration-200 transform whitespace-nowrap  hover:scale-[99%] active:scale-[98%] ${
                viewModeContent === VIEW_MODES.SUBMISSION
                  ? "bg-gradient-to-r from-primary to-primary/80 text-primary-content scale-105"
                  : "text-base-content/60 hover:text-base-content hover:bg-base-200/50"
              }`}
            ...
            <>
              ...
              {videoFiles.length > 0 && (
                <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-4">
                  ...
                  <div
                    className={
                      isMobile ? "min-w-full px-1" : "col-span-2 px-1"
                    }>
                    {selectedVideo ? (
                      <VideoPlayer
              ...
              {assignment.type === "form" && currentUser.role !== "user" && (
                <AssignmentQuestionManager
        ...
        <div className="space-y-4">
          ...
          <div
            ...
            <div className="space-y-2">
              <PropertyItem
                icon={assignment.is_visible ? Globe : Lock}
                label="Visibilitas"
                value={
                  assignment.is_visible ? "Terlihat oleh Siswa" : "Tersembunyi"
                }
                color={assignment.is_visible ? "success" : "warning"}
              />
              <PropertyItem
                icon={Calendar}
                label="Dibuat"
              ...
              {assignment.available_from && (
                <PropertyItem
                  icon={Calendar}
                  label="Tersedia Dari"
                  value={formatDate(assignment.available_from)}
                  color="primary"
                />
              )}
              ...
              <PropertyItem
                icon={User}
                label="Pengajar"
                value={assignment.uploader?.name || "Tidak Diketahui"}
                color="info"
              />
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-base-100 dark:bg-base-200 border border-base-200/50 backdrop-blur-sm shadow-sm">
            ...
            <div className="space-y-2">
              <button
                onClick={() =>
                  navigate(
                    `/classrooms/${classroomCode}/assignment/create?duplicate=${assignmentId}`
                  )
                }
              ...
              <button
                onClick={() =>
                  navigate(
                    `/classrooms/${classroomCode}/assignment/${assignmentId}/history`
                  )
                }
              ...
              <button
                onClick={() => {
                  ...
                  link.href = url;
                  link.download = `assignment-${assignment.id}-export.json`;
                  document.body.appendChild(link);
                  link.click();
                  document.body.removeChild(link);
                  URL.revokeObjectURL(url);
                  setSuccessMessage("Data tugas berhasil diekspor");
                }}
              ...
              <button
                onClick={async () => {
                  const shareData = {
                    title: assignment?.title || "Tugas Pembelajaran",
                    text: `Lihat tugas pembelajaran: ${assignment?.title}`,
                    url: window.location.href,
                  };
                  if (navigator.share) {
                    try {
                      await navigator.share(shareData);
                      setSuccessMessage("Berhasil membagikan tugas");
                    } catch (err) {
              ...
              <button
                ...
                <span>{isDeleting ? "Menghapus..." : "Hapus Tugas"}</span>
              </button>
            </div>
          </div>
        </div>
...
export default memo(AssignmentDetailPage);
