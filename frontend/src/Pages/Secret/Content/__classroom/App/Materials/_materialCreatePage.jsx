...
const MaterialCreatePage = () => {
  ...
  // Hook untuk material operations
  const { addMaterial, createStatus, error, validationErrors } =
  ...
  const draftKey = `material_draft_create_${classroomCode}`;
  ...
  const validateForm = useCallback(() => {
    ...
    // const totalContent = (hasFiles ? files.length : 0) + validLinksCount;
    ...
    //   if (totalContent === 0) {
    //     errors.content =
    //       "Harus memiliki minimal satu mysteriously satu file atau link";
    //   }
    // }
    return errors;
  ...
  const handleSubmit = useCallback(
    async (event) => {
      ...
      try {
        ...
        const result = await addMaterial(submitData);
        if (result.success) {
          setSuccessMessage("Materi berhasil dibuat");
          setIsDirty(false);
          localStorage.removeItem(draftKey);
          navigate(
            `/classrooms/${classroomCode}/material/${result.data.id}/preview`
          );
        } else {
          if (result.error) {
            toast.error(result.error.message || "Gagal membuat materi");
          }
        }
      } catch (err) {
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-base-content/60 mt-2">
                    Tambahkan materi pembelajaran baru untuk kelas ini
                  </p>
                </div>
              </div>
          ...
          <div className="bg-base-100 dark:bg-base-200 backdrop-blur-sm rounded-2xl shadow-sm border border-base-200/50 p-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-[11px] text-base-content/60 mb-4">
                Pilih file untuk ditambahkan ke materi (Maks. 20MB per file)
              </p>
            </div>
          </div>
          ...
          <div className="bg-base-100 dark:bg-base-200 backdrop-blur-sm rounded-2xl shadow-sm border border-base-200/50 p-6">
            ...
            <div className="space-y-3">
              <ActionButton
                ...
                icon={Save}
                label={isSubmitting ? "Menyimpan..." : "Buat Materi"}
                color="success"
                size="md"
                className="w-full"
              ...
              <ActionButton
                onClick={() => {
                  ...
                  saveAs(blob, `material-create-${classroomCode}-draft.xlsx`);
      ...
      <div className="mt-8 bg-base-100 dark:bg-base-200 backdrop-blur-sm rounded-3xl shadow-xl border border-base-300/50 p-6">
        ...
        <div className="space-y-3 text-sm text-base-content/70">
          ...
          <div className="flex gap-3">
            <span className="text-primary font-bold">•</span>
            <span>
              <strong>Tipe yang Tepat:</strong> Pilih tipe materi sesuai dengan
          ...
          <div className="flex gap-3">
            <span className="text-primary font-bold">•</span>
            <span>
              <strong>Jadwal Ketersediaan:</strong> Atur jadwal jika materi
