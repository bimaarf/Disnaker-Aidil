...
const PaymentPreviewPage = () => {
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        const response = await axios.get(`api/payments/${key}`);
        setGame(response.data.payment);
        setLoading(false);
      } catch (err) {
        setLoading(false);
      }
    ...
    <div className="md:flex justify-center md:h-screen items-center">
      ...
      {data.image ? (
        <div className="md:w-2/3 mb-4 flex justify-center">
          <img
            src={`${process.env.REACT_APP_API}payment/images/${data.image}`}
            alt={data.title}
      ...
      <div className="md:w-1/3 border-t md:border-t-transparent md:border-l p-2 border-base-300 md:h-screen overflow-y-auto space-y-2">
        ...
        <div className="text-md pb-20">
          <div
            ...
            className="text-sm whitespace-pre-line prose"
