...
import { createCategoryProduct } from "../../../../../features/product/categoryProductSlice";
...
const CategoryProductCreatePage = () => {
  ...
  const { error } = useSelector((state) => state.categoryProducts);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(createCategoryProduct(formData)).unwrap();
      navigate("/category/product");
      toast.success("CategoryProduct created successfully!");
    } catch (err) {
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <div>
                ...
                <p className={`text-sm ${themeClasses.muted} mt-1`}>
                  Tambahkan kategori product baru
                </p>
              </div>
            </div>
...
export default CategoryProductCreatePage;
