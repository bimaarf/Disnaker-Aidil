...
import {
  fetchCategoryProducts as fetch,
...
const CategoryProductUpdatePage = () => {
  const { key } = useParams();
    ...
    if (key && !dataProps) {
      const fetchData = async () => {
        ...
        try {
          ...
          console.error("Failed to fetch categoryProduct data:", error);
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await dispatch(
        updateCategoryProduct({ key, categoryProductData: formData })
      ).unwrap();
      toast.success("CategoryProduct updated successfully!");
      if (dataProps) {
        navigate(-1);
      } else {
      ...
      toast.error("Failed to update the categoryProduct.");
      console.error("Failed to update the categoryProduct:", error);
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <div>
                ...
                <p className={`text-sm ${themeClasses.muted} mt-1`}>
                  Perbarui informasi kategori product baru
                </p>
              </div>
            </div>
            {loading && (
...
export default CategoryProductUpdatePage;
