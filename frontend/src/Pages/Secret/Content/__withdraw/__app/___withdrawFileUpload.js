...
export const WithdrawFileUpload = ({ selectedData, onUploadSuccess }) => {
  ...
  const handleUpload = () => {
    if (file && selectedData) {
      ...
      dispatch(updateWithdraw({ key: selectedData.key, withdrawData: formData }))
        .then((result) => {
        ...
        .catch(() => {
          toast.error("Failed to update the withdraw.");
        });
    }
  };
